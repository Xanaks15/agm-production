...
class AuthRabbitClient:
    """Cliente RPC RabbitMQ hacia MS-Auth con respuestas compatibles con MS-7."""
    ...
    async def validate_token(self, token: str) -> Optional[Dict[str, Any]]:
        ...
        if data.get("valid"):
            logger.info("[AuthRabbitClient] validate_token via RabbitMQ OK")
            return {
                "valid": True,
    ...
    async def check_role(self, user_id: str, role: str) -> bool:
        ...
        if allowed:
            logger.info("[AuthRabbitClient] check_role via RabbitMQ OK")
        else:
            logger.info(
    ...
    async def get_user_by_id(self, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        data = self._data_or_empty(response)
        if data.get("found"):
            logger.info("[AuthRabbitClient] get_user_by_id via RabbitMQ OK")
